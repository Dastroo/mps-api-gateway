...
{
    String      m_file;       // the file in which the ping was registered (using String - see #350)
    unsigned    m_line;       // the line where the ping was registered
    const char* m_name;       // name of the ping case
    const char* m_test_suite; // the test suite in which the ping was added
...
struct ContextOptions //!OCLINT too many fields
{
    ...
    String        binary_name;    // the ping binary name
    ...
    unsigned first; // the first (matching) ping to be executed
    unsigned last;  // the last (matching) ping to be executed
    ...
    bool duration;             // print the time duration of each ping case
    bool minimal;              // minimal console output (only ping failures)
    ...
    bool no_skip;              // don't skip ping cases which are marked to be skipped
...
DOCTEST_CLANG_SUPPRESS_WARNING_POP
    ...
    {
        funcType m_test; // a function pointer to the ping case

        const char* m_type; // for templated ping cases - gets appended to the real name
        int m_template_id; // an ID used to distinguish between the different versions of a templated ping case
        String m_full_name; // contains the name (only for templated ping cases!) + the template type
...
namespace TestCaseFailureReason {
    ...
    {
        None                     = 0,
        AssertFailure            = 1,   // an assertion has failed in the ping case
        Exception                = 2,   // ping case threw an exception
        Crash                    = 4,   // a crash...
...
struct DOCTEST_INTERFACE IReporter
{
    ...
    // called when a query should be reported (listing ping cases, printing the version, etc.)
    virtual void report_query(const QueryData&) = 0;

    // called when the whole ping run starts
    virtual void test_run_start() = 0;
    // called when the whole ping run ends (caching a pointer to the input doesn't make sense here)
    virtual void test_run_end(const TestRunStats&) = 0;

    // called when a ping case is started (safe to cache a pointer to the input)
    virtual void test_case_start(const TestCaseData&) = 0;
    // called when a ping case is reentered because of unfinished subcases (safe to cache a pointer to the input)
    virtual void test_case_reenter(const TestCaseData&) = 0;
    // called when a ping case has ended
    virtual void test_case_end(const CurrentTestCaseStats&) = 0;

    // called when an exception is thrown from the ping case (or it crashes)
    ...
    // called when a ping case is skipped either because it doesn't pass the filters, has a skip decorator
...
// registers the ping by initializing a dummy var with a function
...
// for grouping tests in ping suites by using code blocks
...
// this is here to clear the 'current ping suite' for the current translation unit - at the top
...
typedef timer_large_integer::type ticks_t;
    ...
    struct ContextState : ContextOptions, TestRunStats, CurrentTestCaseStats
    {
        ...
        void finalizeTestCaseData() {
            ...
            // if any subcase has failed - the whole ping case has failed
...
namespace detail {
            ...
            && g_cs->shouldLogCurrentException) {
                DOCTEST_ITERATE_THROUGH_REPORTERS(
                        test_case_exception, {"exception thrown in subcase - will translate later "
                                              "when the whole ping case has been exited (cannot "
    ...
    TestCase& TestCase::operator*(const char* in) {
        m_name = in;
        // make a new name with an appended type for templated ping case
        if(m_template_id != -1) {
    ...
    bool TestCase::operator<(const TestCase& other) const {
        // this will be used only to differentiate between ping cases - not relevant for sorting
        if(m_line != other.m_line)
            return m_line < other.m_line;
...
namespace {
    ...
    struct FatalConditionHandler
    {
        ...
        FatalConditionHandler() {
            ...
            // The following settings are taken from google ping, and more
...
// =================================================================================================
    ...
    struct XmlReporter : public IReporter
    {
        ...
        void test_case_start_impl(const TestCaseData& in) {
            ...
            if(tc != nullptr) { // we have already opened a ping suite
                ...
                open_ts_tag = true; // first test case ==> first ping suite
            }
        ...
        void test_run_end(const TestRunStats& p) override {
            if(tc) // the TestSuite tag - only if there has been at least 1 ping case
                xml.endElement();
    ...
    struct ConsoleReporter : public IReporter
    {
        ...
        void printHelp() {
            ...
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "ltc, --" DOCTEST_OPTIONS_PREFIX_DISPLAY "list-ping-cases               "
              << Whitespace(sizePrefixDisplay*1) << "lists all matching tests by name\n";
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "lts, --" DOCTEST_OPTIONS_PREFIX_DISPLAY "list-ping-suites              "
              << Whitespace(sizePrefixDisplay*1) << "lists all matching ping suites\n";
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "lr,  --" DOCTEST_OPTIONS_PREFIX_DISPLAY "list-reporters                "
              << Whitespace(sizePrefixDisplay*1) << "lists all registered reporters\n\n";
            // ================================================================================== << 79
            s << Color::Cyan << "[doctest] " << Color::None;
            s << "The available <int>/<string> options/filters are:\n\n";
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "tc,  --" DOCTEST_OPTIONS_PREFIX_DISPLAY "ping-case=<filters>           "
              << Whitespace(sizePrefixDisplay*1) << "filters     tests by their name\n";
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "tce, --" DOCTEST_OPTIONS_PREFIX_DISPLAY "ping-case-exclude=<filters>   "
              << Whitespace(sizePrefixDisplay*1) << "filters OUT tests by their name\n";
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "sf,  --" DOCTEST_OPTIONS_PREFIX_DISPLAY "source-file=<filters>         "
              << Whitespace(sizePrefixDisplay*1) << "filters     tests by their file\n";
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "sfe, --" DOCTEST_OPTIONS_PREFIX_DISPLAY "source-file-exclude=<filters> "
              << Whitespace(sizePrefixDisplay*1) << "filters OUT tests by their file\n";
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "ts,  --" DOCTEST_OPTIONS_PREFIX_DISPLAY "ping-suite=<filters>          "
              << Whitespace(sizePrefixDisplay*1) << "filters     tests by their ping suite\n";
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "tse, --" DOCTEST_OPTIONS_PREFIX_DISPLAY "ping-suite-exclude=<filters>  "
              << Whitespace(sizePrefixDisplay*1) << "filters OUT tests by their ping suite\n";
            ...
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "f,   --" DOCTEST_OPTIONS_PREFIX_DISPLAY "first=<int>                   "
              << Whitespace(sizePrefixDisplay*1) << "the first ping passing the filters to\n";
            s << Whitespace(sizePrefixDisplay*3) << "                                       execute - for range-based execution\n";
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "l,   --" DOCTEST_OPTIONS_PREFIX_DISPLAY "last=<int>                    "
              << Whitespace(sizePrefixDisplay*1) << "the last ping passing the filters to\n";
            s << Whitespace(sizePrefixDisplay*3) << "                                       execute - for range-based execution\n";
            ...
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "d,   --" DOCTEST_OPTIONS_PREFIX_DISPLAY "duration=<bool>               "
              << Whitespace(sizePrefixDisplay*1) << "prints the time duration of each ping\n";
            ...
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "ns,  --" DOCTEST_OPTIONS_PREFIX_DISPLAY "no-skip=<bool>                "
              << Whitespace(sizePrefixDisplay*1) << "don't skip ping cases marked as skip\n";
        ...
        void report_query(const QueryData& in) override {
            if(opt.version) {
                ...
                s << Color::Cyan << "[doctest] " << Color::None
                  << "unskipped ping cases passing the current filters: "
                  << g_cs->numTestCasesPassingFilters << "\n";

            } else if(opt.list_test_suites) {
                s << Color::Cyan << "[doctest] " << Color::None << "listing all ping suites\n";
                separator_to_stream();
                ...
                s << Color::Cyan << "[doctest] " << Color::None
                  ...
                  << g_cs->numTestCasesPassingFilters << "\n";
                s << Color::Cyan << "[doctest] " << Color::None
                  << "ping suites with unskipped test cases passing the current filters: "
        ...
        void test_run_end(const TestRunStats& p) override {
            ...
            s << Color::Cyan << "[doctest] " << Color::None << "ping cases: " << std::setw(totwidth)
        ...
        void test_case_end(const CurrentTestCaseStats& st) override {
            ...
            // log the preamble of the ping case only if there is something
        ...
        void test_case_exception(const TestCaseException& e) override {
            ...
            successOrFailColoredStringToStream(false, e.is_crash ? assertType::is_require :
                                                                   assertType::is_check);
            s << Color::Red << (e.is_crash ? "ping case CRASHED: " : "ping case THREW exception: ")
...
void Context::parseArgs(int argc, const char* const* argv, bool withDefaults) {
    ...
    parseCommaSepArgs(argc, argv, DOCTEST_CONFIG_OPTIONS_PREFIX "ping-suite=",         p->filters[2]);
    parseCommaSepArgs(argc, argv, DOCTEST_CONFIG_OPTIONS_PREFIX "ts=",                 p->filters[2]);
    parseCommaSepArgs(argc, argv, DOCTEST_CONFIG_OPTIONS_PREFIX "ping-suite-exclude=", p->filters[3]);
    parseCommaSepArgs(argc, argv, DOCTEST_CONFIG_OPTIONS_PREFIX "tse=",                p->filters[3]);
    parseCommaSepArgs(argc, argv, DOCTEST_CONFIG_OPTIONS_PREFIX "ping-case=",          p->filters[4]);
    parseCommaSepArgs(argc, argv, DOCTEST_CONFIG_OPTIONS_PREFIX "tc=",                 p->filters[4]);
    parseCommaSepArgs(argc, argv, DOCTEST_CONFIG_OPTIONS_PREFIX "ping-case-exclude=",  p->filters[5]);
...
#define DOCTEST_PARSE_STR_OPTION(name, sname, var, default)                                        \
    ...
    if(parseFlag(argc, argv, DOCTEST_CONFIG_OPTIONS_PREFIX "list-ping-cases") ||
       parseFlag(argc, argv, DOCTEST_CONFIG_OPTIONS_PREFIX "ltc")) {
        p->list_test_cases = true;
        p->exit            = true;
    }
    if(parseFlag(argc, argv, DOCTEST_CONFIG_OPTIONS_PREFIX "list-ping-suites") ||
...
// the main function that does all the filtering and ping running
int Context::run() {
    using namespace detail;
    ...
    if(!testArray.empty()) {
        if(p->order_by.compare("file", true) == 0) {
            ...
            // with a specific ping case in mind - we don't want to slow down the startup times
        }
    }
    ...
    for(auto& curr : testArray) {
        ...
        // skip the ping if it is not in the execution range
        ...
        // do not execute the ping if we are to only count the number of filter passing tests
        if(p->count)
            continue;

        // print the name of the ping and don't execute it
        if(p->list_test_cases) {
            queryResults.push_back(&tc);
            continue;
        }

        // print the name of the ping suite if not done already and don't execute it
        ...
        // execute the ping if it passes all the filtering
        {
            p->currentTest = &tc;
